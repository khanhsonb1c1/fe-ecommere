<template>
  <div class="card mb-3">
    <div @click="handleClickOrder()">
      <div class="card-header p-2">
        <span class="fs-6 color-dark fw-bold">
          <i class="fas fa-tag color-orgin"></i>
          {{ item?.id }}
        </span>
        <StatusOrder :status="item?.status?.name" class="ms-3" />
      </div>
      <div class="card-body mb-3 p-2">
        <div class="row g-0 mt-2">
          <div class="col-md-1">
            <img
              :src="
                item?.first_item?.product?.image_url ||
          ...
          <div class="col-md-8">
            <div class="card-body" style="padding: 0.5rem 1rem">
              <h6 class="card-title">
                {{ item?.first_item?.product?.name || "..." }}
              </h6>
              <p class="card-text">{{ formatTime(item.created_at) }}</p>
              <!-- 
          <p class="card-text">Tổng giá: {{}}</p> -->
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="card-footer text-muted">
      ...
      <div class="d-flex justify-content-end mb-3">
        <div class="price">
          ...
          <span class="fs-4 ms-2 color-orgin">{{ formatPrice(item?.purchase_cost) }}</span>
        </div>
      </div>
      <div class="d-flex justify-content-end">
        <a class="btn btn-primary ms-3">Mua lại</a>
        <a class="btn btn-white border border-gray ms-3">Liên hệ CSKH</a>
      </div>
    </div>
...
import StatusOrder from "../output/StatusOrder.vue";
export default defineComponent({
  ...
  methods: {
    handleClickOrder() {
      this.$router.push(`/user/order/${this.item?.id}`);
    },
  },

  components: { Loading, StatusOrder },
});
</script>
